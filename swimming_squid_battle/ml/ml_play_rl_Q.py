...
MAGNIFY = 300.0  # 放大方向加權數值
WALL_PENALTY = -5.0  # 固定牆邊懲罰
...
class MLPlay:
    def __init__(self, ai_name: str, *args, **kwargs):
        ...
        self.run_counter_path = os.path.join(os.path.dirname(__file__), f"model/run_count_{self.player}.txt")
        ...
        self.reward_log_path = f"ml/model/reward_log_{self.player}.csv"
    ...
    def load_q_table(self):
        path = os.path.join(os.path.dirname(__file__), f"model/q_table_{self.player}.pickle")
        return pickle.load(open(path, "rb")) if os.path.exists(path) else {}

    def get_model_path(self):
        return os.path.join(os.path.dirname(__file__), f"model/q_table_{self.player}.pickle")
    ...
    def get_features(self, scene_info):
        ...
        for obj in scene_info["foods"]:
            ...
            if sx - 75 <= ox <= sx + 75 and sy - 300 <= oy < sy:
                W["UP"] += weight
            elif sx - 75 <= ox <= sx + 75 and sy < oy <= sy + 300:
                W["DOWN"] += weight
            elif sy - 50 <= oy <= sy + 50 and sx - 450 <= ox < sx:
                W["LEFT"] += weight
            elif sy - 50 <= oy <= sy + 50 and sx < ox <= sx + 450:
                W["RIGHT"] += weight

        # 固定牆邊懲罰
        if sy <= 200: W["UP"] += WALL_PENALTY
        if sy >= 450: W["DOWN"] += WALL_PENALTY
        if sx <= 200: W["LEFT"] += WALL_PENALTY
        if sx >= 1000: W["RIGHT"] += WALL_PENALTY
    ...
    def compute_reward(self, scene_info, last_action):
        reward = 0.0
        if scene_info["score"] > self.last_score:
            reward += 10.0  # 吃到食物
