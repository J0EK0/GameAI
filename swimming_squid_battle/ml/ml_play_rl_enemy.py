...
class MLPlay:
    def __init__(self, ai_name: str, *args, **kwargs):
        ...
        self.run_counter_path = os.path.join(os.path.dirname(__file__), f"model_re1/run_count_{self.player}.txt")
        self.run_count = int(open(self.run_counter_path).read()) if os.path.exists(self.run_counter_path) else 0
        self.start_time = datetime.datetime.now()

        self.reward_log_path = f"ml/model_re1/reward_log_{self.player}.csv"
    ...
    def get_model_path(self):
        return os.path.join(os.path.dirname(__file__), f"model_re1/q_table_{self.player}.pickle")
    ...
    def get_features(self, scene_info):
        ...
        def process_object(ox, oy, s):
            dx, dy = ox - sx, oy - sy
            dist = math.hypot(dx, dy) + 1e-5
            scale = 3.0 if dist < 100 else 2.0 if dist < 200 else 1.0
        ...
        for obj in scene_info["foods"]:
            ...
            process_object(ox, oy, s)

        # 處理敵人
        ex, ey = scene_info["opponent_x"], scene_info["opponent_y"]
        my_lv = scene_info["self_lv"]
        op_lv = scene_info["opponent_lv"]
        s=0
        if my_lv > op_lv:
            s = 12
        elif my_lv < op_lv:
            s = -12
        process_object(ex, ey, s)
    ...
    def reset(self):
        ...
        if self.run_count >= 1500:
            print(f"[{self.player}] ✅ Training finished")
            sys.exit(0)
        self.last_state = None
        self.last_action = None
        self.last_score = 0
