...
class ObservationProcessor:
    ...
    def _flatten(self, obs):
        ...
        # === 抽取 Agent 與 Target 的位置 ===
        agent = np.array(obs.get("agent_position", [0, 0, 0]))
        target = np.array(obs.get("target_position", [0, 0, 0]))

        # === Heading Vector (XZ 平面) ===
        heading = target[[0, 2]] - agent[[0, 2]]
        heading_norm = np.linalg.norm(heading) + 1e-6
        heading_unit = heading / heading_norm
        f.extend(heading_unit.tolist())  # (2,)

        # === 距離（normalize 到 30） ===
        dist = np.linalg.norm(target - agent) / 30.0
        f.append(dist)  # (1,)

        # === Agent 的速度與面向方向（XZ 平面）===
        ...
        # === cosine similarity ===
        vel_vec = np.array(forward)
        cos_sim = 0.0
        if heading_norm > 1e-6 and np.linalg.norm(vel_vec) > 1e-6:
            cos_sim = np.dot(vel_vec, heading) / (np.linalg.norm(vel_vec) * heading_norm)
        f.append(cos_sim)  # (1,)
    ...
    def _calculate_observation_size(self) -> int:
        return 2 + 1 + 2 + 2 + 1 + 1  # heading(2) + dist(1) + vel(2) + dir(2) + cos(1) + hp(1)
...
class RewardCalculator:
    def __init__(self):
        ...
        self.angle_weight = 5
        self.stable_threshold = 0.96
        self.stable_bonus = 0.3
        self.min_move_dist = 0.01
        self.dist_improve_thresh = 1.0  # ✨ 距離要縮短超過 1.0 才視為有效改善
    ...
    def calculate(self, obs: Dict[str, Any], base_reward: float, done: bool = False, info: Any = None) -> float:
        ...
        checkpoint_pos = np.array(obs.get("next_checkpoint_pos", [0, 0, 0]))
        ...
        target_vec = checkpoint_pos - self.prev_pos
        target_vec[1] = 0
        target_len = np.linalg.norm(target_vec)

        if move_len > self.min_move_dist and target_len > 1e-3:
            move_unit = move_vec / move_len
            target_unit = target_vec / target_len
            cos_sim = np.clip(np.dot(move_unit, target_unit), -1.0, 1.0)

            # === 距離差判斷 ===
            prev_dist = np.linalg.norm(checkpoint_pos - self.prev_pos)
            new_dist = np.linalg.norm(checkpoint_pos - agent_pos)
            dist_delta = prev_dist - new_dist

            # ✅ 有效角度 + 有效距離靠近 才加分
            if cos_sim > 0.8 and dist_delta > self.dist_improve_thresh:
                print(f"facing{dist_delta}")
                r += cos_sim * self.angle_weight * 2
            elif cos_sim > 0.5:
                r += cos_sim * self.angle_weight
            elif cos_sim < -0.3 and dist_delta < -0.5:
                r -= 1

            # ✅ 穩定移動方向
            if self.prev_dir is not None:
                dir_sim = np.dot(move_unit, self.prev_dir)
                if dir_sim > self.stable_threshold:
                    self.stable_count += 1
                    r += self.stable_bonus
                else:
                    self.stable_count = 0
            self.prev_dir = move_unit
        ...
        if cur_cp > self.prev_checkpoint:
            ...
            print(f"[reward_debug] ✅ reached checkpoint {cur_cp}")
