...
class MLPlay:
    ...
    def update(self, scene_info, *args, **kwargs):
        if scene_info["status"] != "GAME_ALIVE":
            # Still need to record the final game status
            self.data.append({
                ...
                "command": "NONE",
                "predicted_x": 100,  # No need to predict landing point here
        ...
        # === Convert to 1P perspective if current side is 2P ===
        if self.side == "2P":
            ball_y = 500 - ball_y
            ball_dy = -ball_dy
        ...
        # Simulate landing point and determine if it hits the blocker
        if ball_to_me:
            predicted_x, will_hit_blocker = self.simulate_landing((ball_x, ball_y), (ball_dx, ball_dy), (blocker_x, blocker_y), landing_y)
        elif ball_from_me and ball_y < half_y:
            predicted_x, will_hit_blocker = self.simulate_landing((ball_x, ball_y), (-ball_dx, -ball_dy), (blocker_x, blocker_y), landing_y)
        else:
            predicted_x, will_hit_blocker = self.simulate_landing((ball_x, ball_y), (ball_dx, -ball_dy), (blocker_x, blocker_y), landing_y)

        # Offset prediction only if it will hit the blocker and platform has enough room
